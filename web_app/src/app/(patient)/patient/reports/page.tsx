...
function ReportDetailView({ id }: { id: string }) {
  ...
  return (
    <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-4 bg-gray-50/30 dark:bg-slate-950/30">
        ...
        {editing ? (
          ...
          <div className="flex items-start justify-between">
            ...
            <div className="flex items-center gap-1 ml-2">
              <button onClick={() => setEditing(true)} className="p-1.5 text-primary-600 hover:bg-primary-50 dark:hover:bg-primary-900/30 rounded-lg transition-colors flex-shrink-0" title="Edit title">
              ...
              <button onClick={handleDelete} className="p-1.5 text-red-600 hover:bg-red-50 dark:hover:bg-red-900/30 rounded-lg transition-colors flex-shrink-0" title="Delete report">
      ...
      {/* Extracted Data Card */}
      {(report.ocr_status === 'extracted' || report.ocr_status === 'verified') && (
        <div className="bg-white dark:bg-slate-900 rounded-2xl border border-gray-100 dark:border-slate-800 shadow-sm overflow-hidden">
          ...
          <div className="p-4 space-y-6">
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              {['test_name', 'lab_name', 'patient_name', 'report_date', 'doctor_name'].map((key) => {
                const val = extracted ? extracted[key] : null;
                return (
                  <div key={key} className="bg-white dark:bg-slate-900 border border-gray-100 dark:border-slate-800 rounded-xl p-3 shadow-sm">
                    <p className="text-[10px] font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-widest">{humanizeSnake(key)}</p>
                    <p className="text-sm font-medium text-gray-900 dark:text-gray-100 mt-1">{val ? String(val) : '—'}</p>
                  </div>
                );
              })}
            </div>
              ...
              <div className="border border-gray-200 dark:border-slate-800 rounded-xl overflow-hidden shadow-sm">
                <div className="overflow-x-auto">
                  <table className="w-full text-sm text-left whitespace-nowrap">
                    <thead className="bg-gray-50 dark:bg-slate-950 text-gray-600 dark:text-gray-300 border-b border-gray-200 dark:border-slate-800">
                      <tr>
