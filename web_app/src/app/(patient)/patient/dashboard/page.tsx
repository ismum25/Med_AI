  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6">
      ...
      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-primary-600 to-primary-800 px-8 py-7 text-white">
        ...
        <div className="absolute right-16 -bottom-8 h-20 w-20 rounded-full bg-white/[0.06]" />
      ...
      <div className="grid grid-cols-3 gap-3">
        <Link href="/patient/appointments/book" className="flex flex-col items-center gap-2 rounded-2xl border border-primary-100 bg-primary-50/50 py-4 hover:bg-primary-50 transition-colors group">
      ...
      <div className="grid grid-cols-3 gap-3">
        {[
          { value: upcoming.length, label: 'Upcoming', color: 'text-primary-600 bg-primary-50', icon: '📅' },
          ...
          <div key={s.label} className="rounded-2xl bg-white border border-gray-100 shadow-sm p-4 flex items-center gap-3">
      ...
      <div>
        ...
        {nextAppt ? (
          <Link href="/patient/appointments" className="block rounded-2xl bg-white border border-gray-100 shadow-sm p-4 hover:shadow-md transition-shadow">
            <div className="flex items-center gap-4">
              ...
              <div className="min-w-0 flex-1">
                <p className="font-semibold text-gray-900 truncate">{nextAppt.reason || 'Consultation'}</p>
                <p className="text-sm text-gray-500">{formatTime(nextAppt.scheduled_at)}, {formatDate(nextAppt.scheduled_at, 'EEEE')}</p>
              </div>
          ...
          <div className="rounded-2xl bg-white border border-gray-100 shadow-sm p-4 flex items-center gap-4">
            ...
            <div className="flex-1">
              ...
              <p className="text-sm text-gray-500">Book one to get started</p>
            </div>
      ...
      <div>
        ...
        {reports.length === 0 ? (
          <div className="rounded-2xl bg-white border border-gray-100 shadow-sm p-4 flex items-center gap-4">
            ...
            <p className="flex-1 text-gray-500">No reports yet</p>
            <Link href="/patient/reports/upload" className="text-sm font-medium text-primary-600 hover:underline">Upload</Link>
          </div>
        ) : (
          <div className="flex gap-3 overflow-x-auto pb-2">
            {reports.slice(0, 4).map((r) => (
              <Link key={r.id} href={`/patient/reports/${r.id}`} className="flex-shrink-0 w-36 rounded-2xl bg-white border border-gray-100 shadow-sm p-3 hover:shadow-md transition-shadow">
                <span className="text-primary-600">📄</span>
                <p className="text-sm font-semibold text-gray-900 mt-2 truncate">{r.title || r.file_name || 'Report'}</p>
