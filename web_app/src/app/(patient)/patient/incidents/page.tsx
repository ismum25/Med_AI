    ...
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      {incidents.length === 0 ? (
        <div className="text-center py-16">
          ...
          <p className="text-gray-500 dark:text-gray-400 dark:text-gray-500 mb-1">No incidents yet</p>
          <p className="text-sm text-gray-400 dark:text-gray-500 mb-4">Upload an injury photo to get an AI assessment</p>
        ...
        <div className="space-y-3">
          {incidents.map((inc) => (
            <Link key={inc.id} href={`/patient/incidents/${inc.id}`} className="block rounded-xl bg-white dark:bg-slate-900 border border-gray-100 dark:border-slate-800 shadow-sm p-4 hover:shadow-md transition-shadow">
              <div className="flex items-center gap-4">
                <IncidentThumb id={inc.id} />
                <div className="flex-1 min-w-0">
                  <p className="font-semibold text-gray-900 dark:text-gray-100 truncate">{inc.title || 'Incident'}</p>
