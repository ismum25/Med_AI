  ...
  return (
    <div className="max-w-5xl mx-auto space-y-8">
        ...
        {STATUS_TABS.map((s) => (
          <button
            ...
            className={`flex-1 px-3 py-1.5 rounded-md text-sm font-medium transition-colors ${
              tab === s ? 'bg-white dark:bg-slate-900 text-gray-900 dark:text-gray-100 shadow-sm' : 'text-gray-500 dark:text-gray-400 dark:text-gray-500 hover:text-gray-700 dark:text-gray-300'
            }`}
          >
            {capitalize(s)}
          </button>
      ...
      <div className="space-y-3">
        {filtered.length === 0 ? (
          <div className="text-center py-12 text-gray-500 dark:text-gray-400 dark:text-gray-500">No {tab === 'all' ? '' : tab} appointments</div>
        ) : (
          filtered.map((a) => (
            <div key={a.id} className="rounded-xl bg-white dark:bg-slate-900 border border-gray-100 dark:border-slate-800 shadow-sm p-4">
              <div className="flex items-center gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <p className="font-semibold text-gray-900 dark:text-gray-100 truncate">{a.reason || 'Consultation'}</p>
                  <p className="text-sm text-gray-500 dark:text-gray-400 dark:text-gray-500">{formatTime(a.scheduled_at)}, {formatDate(a.scheduled_at, 'EEEE, MMM d')}</p>
                </div>
      ...
      {/* Find a Doctor */}
      <div>
        <h2 className="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-4">Find a Doctor</h2>
        <div className="mb-4">
          <select
            value={selectedSpec}
            onChange={(e) => setSelectedSpec(e.target.value)}
            className="border border-gray-200 dark:border-slate-800 rounded-lg px-3 py-2 text-sm bg-white dark:bg-slate-900 focus:outline-none focus:ring-2 focus:ring-primary-500"
          >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {filteredDoctors.map((d) => (
            <Link key={d.user_id} href={`/patient/appointments/doctor/${d.user_id}`} className="rounded-xl bg-white dark:bg-slate-900 border border-gray-100 dark:border-slate-800 shadow-sm p-4 hover:shadow-md transition-shadow">
              <div className="flex items-center gap-3">
                ...
                <div className="min-w-0">
                  <p className="font-semibold text-gray-900 dark:text-gray-100 truncate">{d.full_name}</p>
