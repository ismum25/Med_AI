  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="rounded-2xl bg-white dark:bg-slate-900 border border-gray-100 dark:border-slate-800 shadow-sm p-4 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-gray-500 dark:text-gray-400 dark:text-gray-500">Today&apos;s Appointments</p>
          </div>
        </div>
        <div className="rounded-2xl bg-white dark:bg-slate-900 border border-gray-100 dark:border-slate-800 shadow-sm p-4 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-gray-500 dark:text-gray-400 dark:text-gray-500">Pending Reviews</p>
          </div>
        </div>
      </div>
      ...
      <div>
        ...
        {today.length === 0 ? (
          <div className="rounded-xl bg-white dark:bg-slate-900 border border-gray-100 dark:border-slate-800 shadow-sm p-4 text-center text-gray-500 dark:text-gray-400 dark:text-gray-500 text-sm">
            ...
            {today.slice(0, 5).map((a) => (
              <div key={a.id} className="rounded-xl bg-white dark:bg-slate-900 border border-gray-100 dark:border-slate-800 shadow-sm p-4 flex items-center gap-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-sm text-gray-500 dark:text-gray-400 dark:text-gray-500">{formatTime(a.scheduled_at)} • {a.reason || 'Consultation'}</p>
                </div>
              ...
              <Link key={r.id} href={`/doctor/review/${r.id}`} className="block rounded-xl bg-white dark:bg-slate-900 border border-gray-100 dark:border-slate-800 shadow-sm p-4 hover:shadow-md transition-shadow">
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-sm text-gray-500 dark:text-gray-400 dark:text-gray-500 capitalize">{r.report_type?.replace(/_/g, ' ') || 'Report'}</p>
                  </div>
