  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6">
      ...
      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-primary-600 to-primary-800 px-8 py-7 text-white">
        ...
        <div className="mt-3 inline-flex items-center gap-1.5 rounded-full bg-white/[0.18] border border-white/[0.28] px-3.5 py-1.5 text-xs font-medium">
      ...
      <div className="grid grid-cols-3 gap-3">
        <Link href="/doctor/appointments" className="flex flex-col items-center gap-2 rounded-2xl border border-primary-100 bg-primary-50/50 py-4 hover:bg-primary-50 transition-colors">
          <span className="text-xl">📅</span>
          <span className="text-xs font-semibold text-primary-600">Schedule</span>
        </Link>
        <Link href="/doctor/review" className="flex flex-col items-center gap-2 rounded-2xl border border-violet-100 bg-violet-50/50 py-4 hover:bg-violet-50 transition-colors">
          <span className="text-xl">📋</span>
          <span className="text-xs font-semibold text-violet-600">Review</span>
        </Link>
        <Link href="/doctor/patients" className="flex flex-col items-center gap-2 rounded-2xl border border-teal-100 bg-teal-50/50 py-4 hover:bg-teal-50 transition-colors">
          <span className="text-xl">👥</span>
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="rounded-2xl bg-white border border-gray-100 shadow-sm p-4 flex items-center gap-3">
          <span className="text-lg w-10 h-10 rounded-xl bg-primary-50 text-primary-600 flex items-center justify-center">📅</span>
          <div>
        ...
        <div className="rounded-2xl bg-white border border-gray-100 shadow-sm p-4 flex items-center gap-3">
          <span className="text-lg w-10 h-10 rounded-xl bg-violet-50 text-violet-600 flex items-center justify-center">📋</span>
          <div>
      ...
      <div>
        ...
        {today.length === 0 ? (
          <div className="rounded-xl bg-white border border-gray-100 shadow-sm p-4 text-center text-gray-500 text-sm">
            ...
            {today.slice(0, 5).map((a) => (
              <div key={a.id} className="rounded-xl bg-white border border-gray-100 shadow-sm p-4 flex items-center gap-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-sm text-gray-500">{formatTime(a.scheduled_at)} • {a.reason || 'Consultation'}</p>
                </div>
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          <h2 className="text-lg font-semibold text-gray-900">Reports Awaiting Review</h2>
            ...
            {pendingReports.slice(0, 5).map((r) => (
              <Link key={r.id} href={`/doctor/review/${r.id}`} className="block rounded-xl bg-white border border-gray-100 shadow-sm p-4 hover:shadow-md transition-shadow">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-xl bg-violet-50 text-violet-600 flex items-center justify-center flex-shrink-0">📄</div>
                  <div className="flex-1 min-w-0">
                    <p className="font-semibold text-gray-900 truncate">{r.title || r.file_name || 'Medical Report'}</p>
                    <p className="text-sm text-gray-500 capitalize">{r.report_type?.replace(/_/g, ' ') || 'Report'}</p>
                  </div>
